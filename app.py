...
st.markdown("<hr style='margin: -10px -10px -10px -10px; border-color: #333;'>", unsafe_allow_html=True)
...
with col_sim:
    simulated_now = pd.to_datetime(f"{selected_date} {selected_hour}:00:00").tz_localize('UTC')
...
# BACKEND ML PIPELINE
# ==========================================
live_df = df[(df["created_datetime"] <= simulated_now) & (df["created_datetime"] >= cutoff_time)].copy()
...
if app_view == "OPERATE":
    ...
    with col_left:
        with st.container(height=CONTAINER_HEIGHT):
            ...
            # 2. Group by the hour of the day (0-23)
            hourly_groups = df.groupby(df['created_datetime'].dt.hour)

            # 3. Count the number of rows (vehicles detected) in each hour
            hourly_counts = hourly_groups.size().reset_index(name='vehicle_count')

            # Rename the groupby output column to 'hour' for clarity
            hourly_counts.rename(columns={'created_datetime': 'hour'}, inplace=True)

            # 4. Add the color column (Red for peak hours 9-12, Blue for others)
            hourly_counts['color'] = np.where((hourly_counts['hour'] >= 9) & (hourly_counts['hour'] <= 12), '#ff4b4b', '#00a4ff')

            # 5. Build and render the Altair chart using the new hourly_counts dataframe
            chart = alt.Chart(hourly_counts).mark_bar().encode(
                x=alt.X('hour:O', title=None, axis=alt.Axis(labelAngle=0, labelColor='#8a8d93')),
                y=alt.Y('vehicle_count:Q', title=None, axis=alt.Axis(labelColor='#8a8d93')),
            ...
            with st.container(height=230):
                for idx, row in live_df.sort_values("created_datetime", ascending=False).head(20).iterrows():
                    viol_type = str(row.get('violation_type', 'UNKNOWN')).upper()[:15]
                    time_str = row['created_datetime'].strftime('%I:%M %p')
